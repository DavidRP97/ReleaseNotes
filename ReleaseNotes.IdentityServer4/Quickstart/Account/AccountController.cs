...
using ReleaseNotes.IdentityServer4.Quickstart.Account;
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var loginresult = await _signInManager.PasswordSignInAsync(model.Email, model.Password, model.AllowRememberLogin, lockoutOnFailure: true);

                    if (loginresult.Succeeded)
                    {
                        var checkuser = await _userManager.FindByNameAsync(model.Email);
                        await _events.RaiseAsync(new UserLoginSuccessEvent(checkuser.Email, checkuser.Id, checkuser.Email, clientId: context?.Client.ClientId));
